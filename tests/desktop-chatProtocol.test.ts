...
describe("chat protocol", () => {
  ...
  it("advertises a fixed event vocabulary", () => {
    expect(CHAT_EVENT_TYPES).toEqual(["text_delta", "assistant_text", "tool_use", "tool_result", "notice", "error", "permission_request", "user_text", "complete", "new_session", "session_id", "theme", "status", "statusline_picker", "done"]);
  });
