...
describe("desktop shell split", () => {
  it("preload exposes chat IPC and no terminal IPC", () => {
    const source = readFileSync("src/desktop/shell/preload.cjs", "utf8");
...
describe("slash parity", () => {
  it("every GUI-visible command is reachable from the chat autocomplete", () => {
    const source = readFileSync("src/desktop/renderer/chatPane.tsx", "utf8");
  ...
  it("renderer stays browser-safe: no node:* or backend imports in the Vite bundle", () => {
    ...
    for (const file of files) {
      const source = readFileSync(`src/desktop/renderer/${file}`, "utf8");
    ...
    expect(readFileSync("src/desktop/renderer/themeState.ts", "utf8")).toContain("ui/builtinThemes.js");
    expect(readFileSync("src/desktop/renderer/themeMenu.tsx", "utf8")).toContain("ui/builtinThemes.js");
    expect(readFileSync("src/desktop/renderer/chatPane.tsx", "utf8")).toContain("commands/guiSlashNames.js");
  ...
  it("desktop theme switching lives in the titlebar menu with preview semantics, not the input", () => {
    ...
    const menu = readFileSync("src/desktop/renderer/themeMenu.tsx", "utf8");
    ...
    const themeState = readFileSync("src/desktop/renderer/themeState.ts", "utf8");
  ...
  it("argument completion round-trips through the backend and preserves the prefix", () => {    const pane = readFileSync("src/desktop/renderer/chatPane.tsx", "utf8");
  ...
  it("statusline footer mirrors the TUI segments with backend polling", () => {
    const bar = readFileSync("src/desktop/renderer/statusBar.tsx", "utf8");
    expect(bar).toContain("formatStatusSegments");
    expect(bar).toContain("StatuslinePicker");
    const shell = readFileSync("src/desktop/renderer/src.tsx", "utf8");
