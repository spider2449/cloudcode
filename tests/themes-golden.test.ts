...
// Pins resolver output for all builtins in both modes. During the
...
// The registration pipeline: template fill-in before resolution (mirrors
// registerTheme in theme.ts).
  ...
  for (const mode of ["dark", "light"] as const) {
    it(`every builtin resolves identically to the golden fixture (${mode})`, () => {
      for (const [name, json] of Object.entries(BUILTIN_THEME_JSONS)) {
        ...
        const resolved = resolveThemeJson({ ...json, theme: withStandardRoles(json.theme, json.defs) }, mode);
